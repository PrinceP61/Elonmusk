    ...
    if 2 < len(message.text) < 50:    
        ...
        if files:
            for file in files:
                ...
                filename = f"[{get_size(file.file_size)}] {file.file_name}"
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}", url=t="https://du-link.in/st?api=a06665469dd939aba1806e961f9a185549403055&url=https://telegram.dog/M2LiNKS_FilterBot?start=pr0fess0r_99_-_-_-_BAADAQADLAIAAs-XqEVRb1Rk-gcJ8xYE"print(t.rsplit("&url=",1)[-1])])        else:
            return
        if not btn:
            return
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        await query.answer("Seriously 🙄 ?",show_alert=True)
