        ...
        if files:
            for file in files:
                ...
                btn.append(
                    ...
                    [InlineKeyboardButton(text=f"{url=f"https://api.shareus.in/directLink?token=xGzniHOkadfY4aP4HKbnGxHGGkv1&link=https://telegram.dog/{nyva}?start=pr0fess0r_99_-_-_-_{file_id}")])
        else:
            return
        if not btn:
            return
